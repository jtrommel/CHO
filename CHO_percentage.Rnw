...
universiteiten_mast <- read.csv("Data/universiteiten_mast.csv", header=FALSE, sep=";",col.names=c("Naam","Instelling","Locatie","Onderwijstaal"), dec=",", na.strings="NA", stringsAsFactors = TRUE)
@

<<>>=
hogescholen %>% group_by(Onderwijstaal) %>% count()
universiteiten_bach %>% group_by(Onderwijstaal) %>% count()
universiteiten_mast %>% group_by(Onderwijstaal) %>% count()
@
